...
public class BraintreeMerchantService
{
    ...
    private List<BraintreeMerchantRecord> GetMerchantsByStoreId(int storeId)
    {
        return _staticCacheManager.Get(
            _staticCacheManager.PrepareKeyForDefaultCache(_braintreeserviceExistsCurrencyKey, storeId),
    ...
    private void UpdateTable(int storeId)
    {
        ...
        using (new ReaderWriteLockDisposable(_locker))
        {
            ...
            _staticCacheManager.RemoveAsync(_staticCacheManager.PrepareKeyForDefaultCache(_braintreeserviceExistsCurrencyKey, storeId)).Wait();
        }
    }
    ...
    public async Task UpdateMerchantAsync(int id, string merchantAccountId)
    {
        ...
        await _staticCacheManager.RemoveAsync(_staticCacheManager.PrepareKeyForDefaultCache(_braintreeserviceMerchantKey, merchant.CurrencyCode, merchant.StoreId));
    }
    ...
    public async Task<string> GetMerchantIdAsync(string currencyCode)
    {
        var merchant = await _staticCacheManager.GetAsync(
            _staticCacheManager.PrepareKeyForDefaultCache(_braintreeserviceMerchantKey, currencyCode, (await _storeContext.GetCurrentStoreAsync()).Id), () =>
            {
